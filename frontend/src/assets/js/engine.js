...
class FarmingEngine {
  ...
  async initWeather(customLoc = null) {
    try {
      this.weather = await window.WEATHER_DATA.generateWeatherRealTime(customLoc || this.profile.location);
    } catch (e) {
  ...
  // ── Today's Tasks (weather-aware & domain-filtered) ──
  getTasks() {
    let rawTasks = window.FARM_DATA.getTasksForProfile(this.profile);
    const types = this.profile.farmTypes || ['crop'];
    
    // Dynamically insert high-priority weather-based tasks strictly tailored to selected domains
    if (this.weather.rainTomorrow) {
      if (types.includes('bees')) {
        rawTasks.unshift({ id:'wt_rain_bees', text:'Heavy rain expected tomorrow → check apiary anchors & secure hive covers against drafts', icon:'🐝', time:'Today', priority:'high' });
      } else if (types.includes('fish')) {
        rawTasks.unshift({ id:'wt_rain_fish', text:'Rain expected tomorrow → check pond outlet nets & spillway to prevent runoff loss', icon:'🐟', time:'Today', priority:'high' });
      } else if (types.includes('cattle')) {
        rawTasks.unshift({ id:'wt_rain_cattle', text:'Rain expected tomorrow → ensure cattle shed drainage is clear and dry fodder stored indoors', icon:'🐄', time:'Today', priority:'high' });
      } else if (types.includes('poultry')) {
        rawTasks.unshift({ id:'wt_rain_poultry', text:'Rain expected tomorrow → check poultry roof integrity and replace damp litter', icon:'🐔', time:'Today', priority:'high' });
      } else if (types.includes('crop') || types.includes('organic')) {
        if (this.profile.experience === 'expert') {
          rawTasks.unshift({ id:'wt_rain_e', text:'Rain expected tomorrow → delay irrigation and pesticide/fertilizer application', icon:'🌧️', time:'Next 24h', priority:'high' });
        } else {
      ...
      if (types.includes('bees')) {
        rawTasks.unshift({ id:'wt_heat_bees', text:'Extreme heat today → place shade boards over hives and provide fresh water nearby', icon:'🌡️', time:'Morning', priority:'high' });
      } else if (types.includes('cattle')) {
        rawTasks.unshift({ id:'wt_heat_cattle', text:'High heat index → provide ample cool drinking water with electrolytes & shade for cows', icon:'🔥', time:'Morning', priority:'high' });
      } else if (types.includes('fish')) {
        rawTasks.unshift({ id:'wt_heat_fish', text:'Hot sunny day → operate aerators in early morning to prevent dissolved oxygen depletion', icon:'🌊', time:'Early AM', priority:'high' });
      } else if (types.includes('crop') || types.includes('organic')) {
        rawTasks.unshift({ id:'wt_heat_crop', text:'Very hot today → schedule early morning irrigation before 8 AM to prevent crop wilting', icon:'🔥', time:'Morning', priority:'high' });
      }
    }
    ...
    if (this.profile.specificItems && typeof this.profile.specificItems === 'string' && this.profile.specificItems.trim() !== '') {
      ...
      items.forEach((item, index) => {
        if (this.profile.experience === 'expert') {
          rawTasks.unshift({ id:'custom_'+index, text:`Monitor parameters and market prices for your custom species/crop: ${item.toUpperCase()}`, icon:'🔍', time:'Regularly', priority:'medium' });
        } else {
          rawTasks.unshift({ id:'custom_'+index+'_b', text:`Check your ${item} carefully today for optimal health and growth conditions.`, icon:'🔍', time:'Today', priority:'medium' });
        }
      });
    }
  ...
  // ── All Alerts + Disease (strictly filtered by farmTypes) ──
  getAllAlerts() {
    const { farmTypes } = this.profile;
    const weatherAlerts  = window.WEATHER_DATA.getWeatherAlerts(this.weather, farmTypes);
